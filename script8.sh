...
# 08 - Verificando se a URL está acessível antes de tentar baixar
echo "> VERIFICANDO SE A URL ESTÁ ACESSÍVEL"
if ! wget --spider -q --user-agent="Mozilla/5.0" "$URL_TEMPLATE"; then
    echo "Erro: A URL não está acessível. Verifique a conexão ou o link."
    exit 1
fi

# 09 - Baixando o template com um cabeçalho de User-Agent
...
# 10 - Instalando o unzip (se necessário)
echo "> INSTALANDO O UNZIP"
apt install unzip -y

# 11 - Descompactando o template diretamente no repositório do Apache
...
# 13 - Habilitando o Apache para iniciar automaticamente no boot
echo "> HABILITANDO O APACHE PARA INICIAR NO BOOT"
systemctl enable apache2

# 14 - Verificando se o Apache está rodando
